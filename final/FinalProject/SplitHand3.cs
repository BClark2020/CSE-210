public class SplitHandThree : Hand
{
	...
	public override void Main(int _bet, List<string> _splittingHand)
	{
		...
		_handThree.Clear();
		_handFour.Clear();
		game._splitHandBool = false;
		_bet = 0;
		if (split2._handOne == _splittingHand)
		{
		...
		else if (split2._handTwo == _splittingHand)
		{
		...
		int _round = 0;
		Console.Clear();
		_handFour = CreateHands(_splittingHand, _bet);
		game._continue = true;
		if (_theeHand == 1)
		{
